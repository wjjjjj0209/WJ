...
class Generator(Model):
    def __init__(self):
        ...
        self.f2 = Dense(4 * 4 * 512, activation=tf.nn.leaky_relu)
...
def getfacedata():
    if os.path.exists('./celebA_hq/celeba-64/facedata64.npy'):
        x = np.load('./celebA_hq/celeba-64/facedata64.npy')
    else:
        listimg = os.listdir('./celebA_hq/celeba-64/celeba-64/')
        x = []
        for i in range(len(listimg)):
            img = Image.open('./celebA_hq/celeba-64/celeba-64/' + listimg[i])
            x.append(np.array(img))
        x = np.array(x)
        np.save('./celebA_hq/celeba-64/facedata64.npy', x)
...
def main():
    ...
    if os.path.exists('./GAN_VGG/generatorweights/gweights.index'):
        print('-------------load the model-----------------')
        generator.load_weights('./GAN_VGG/generatorweights/gweights')
        discriminator.load_weights('./GAN_VGG/discriminatorweights/dweights')
    ...
    generator.save_weights('./GAN_VGG/generatorweights/gweights')
    discriminator.save_weights('./GAN_VGG/discriminatorweights/dweights')
    ...
    for i in range(10):
        ...
        g_sample = np.reshape(g_sample, newshape=(4, 64, 64, 3)).astype(np.int)
        # 把40张图片画出来
        for j in range(4):
