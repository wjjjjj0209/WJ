...
class Generator(Model, ABC):
    ...
    def gerenate(self, inputs, level, transit, trans_alpha):
        x = self.f1(inputs)
...
class Discriminator(Model, ABC):
    ...
    def __init__(self, levels):
        ...
        self.c2 = Conv2D(filters=512, kernel_size=4, activation=tf.nn.leaky_relu,
                         kernel_initializer=tf.keras.initializers.he_normal)
        self.f1 = Flatten()
        self.f2 = Dense(1, kernel_initializer=tf.keras.initializers.he_normal)

    def discriminate(self, inputs, level, transit, trans_alpha):
        rgb0 = inputs
